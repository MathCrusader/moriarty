...
#ifndef MORIARTY_CONSTRAINTS_EQUALITY_CONSTRAINTS_H_
#define MORIARTY_CONSTRAINTS_EQUALITY_CONSTRAINTS_H_
...
template <typename T>
Dependencies Exactly<T>::GetDependencies() const {
  return {};
}
...
std::string OneOf<T>::ToString() const {
  return std::format("is one of {}", moriarty_internal::ValuePrinter(options_));
}

template <typename T>
ValidationResult OneOf<T>::Validate(ConstraintContext ctx,
                                    const T& value) const {
...
template <typename T>
Dependencies OneOf<T>::GetDependencies() const {
  return {};
}

}  // namespace moriarty

#endif  // MORIARTY_CONSTRAINTS_EQUALITY_CONSTRAINTS_H_
