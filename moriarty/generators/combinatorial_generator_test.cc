...
#include "moriarty/internal/combinatorial_coverage.h"
#include "moriarty/internal/combinatorial_coverage_test_util.h"
...
TEST(CombinatorialCoverage, GenerateShouldCreateCasesFromCoveringArray) {
  ...
  for (const MTestCase& test_case : test_cases) {
    generated_cases.push_back(moriarty_internal::GenerateAllValues(
        context.Variables(), test_case.UnsafeGetVariables(),
        test_case.UnsafeGetValues(), {rng}));
  }
