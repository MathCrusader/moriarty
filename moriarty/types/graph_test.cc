...
TEST(GraphTest, ConstructorGivesEmptyGraph) {
  Graph graph(10);
  EXPECT_THAT(graph.GetNodeLabels(), AllOf(SizeIs(10), Each(NoNodeLabel{})));
...
TEST(GraphTest, NodeLabelsCanBeSetAndGot) {
  Graph<NoEdgeLabel, int> graph(10);
...
TEST(GraphTest, AddEdgeAddsEdge) {
  ...
  EXPECT_THAT(graph.GetAdjacencyList(),
              ElementsAre(IsEmpty(),                               //
                          IsEmpty(),                               //
                          ElementsAre(Graph<int>::Edge{2, 3, 5}),  //
                          ElementsAre(Graph<int>::Edge{3, 2, 5})));
  EXPECT_THAT(graph.GetEdges(), ElementsAre(Graph<int>::Edge{2, 3, 5}));
}
