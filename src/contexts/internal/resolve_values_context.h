...
#ifndef MORIARTY_CONTEXTS_INTERNAL_RESOLVE_VALUES_CONTEXT_H_
#define MORIARTY_CONTEXTS_INTERNAL_RESOLVE_VALUES_CONTEXT_H_
...
T::value_type ResolveValuesContext::GenerateVariable(
  ...
  if (values_.get().Contains(variable_name)) {
    if (auto v =
            extra_constraints.Validate({variable_name, variables_, values_},
                                       values_.get().Get<T>(variable_name));
        !v.IsOk()) {
      throw GenerationError(
          variable_name,
          std::format("Value is already known, but does not satisfy "
...
#endif  // MORIARTY_CONTEXTS_INTERNAL_RESOLVE_VALUES_CONTEXT_H_
