...
#ifndef MORIARTY_SRC_UTIL_STATUS_BUILDER_H_
#define MORIARTY_SRC_UTIL_STATUS_BUILDER_H_
...
class [[nodiscard]] StatusBuilder {
 public:
  ...
  explicit StatusBuilder(const absl::Status& original_status,
                         SourceLocation location
                             MORIARTY_LOC_CURRENT_DEFAULT_ARG);
  ...
  //
  //   MORIARTY_RETURN_IF_ERROR(foo()).With(TeamPolicy).With(OtherTeamPolicy);
  //
  // Terminal adaptors are often used for type conversion. This allows
  // MORIARTY_RETURN_IF_ERROR to be used in functions which do not return
  // Status. For example, a function might want to return some default value on
  // error:
  //
  //   int GetSysCounter() {
  //     int value;
  //     MORIARTY_RETURN_IF_ERROR(ReadCounterFile(filename, &value))
  //         .LogInfo()
  ...
  //
  //   bool DoMyThing() {
  //     MORIARTY_RETURN_IF_ERROR(foo()).LogWarning().With(status_macros::Return(false));
  //     ...
  //   }
  //
  ...
  //     int64 id;
  //     MORIARTY_RETURN_IF_ERROR(GetIdForName(name,
 ...
 private:
  ...
  struct Rep {
    ...
    moriarty::moriarty_internal::OStringStream stream{&stream_message};
...
StatusBuilder AbortedErrorBuilder(
    SourceLocation location MORIARTY_LOC_CURRENT_DEFAULT_ARG);
...
//
//   MORIARTY_RETURN_IF_ERROR(foo(val))
...
//       .With(util::TaskReturn(task));
//
// Note in the above example, the MORIARTY_RETURN_IF_ERROR macro ensures the
...
class ExtraMessage {
 ...
 private:
  std::string msg_;
  moriarty::moriarty_internal::OStringStream stream_;
};
...
#endif  // MORIARTY_SRC_UTIL_STATUS_BUILDER_H_
